...
## DASH APP CODE
# The layout div has four child divs. The first div
# has the first row of dropdown menus, the second div
# has the second row of dropdown menus. The third div
# contains the plot and sidebar text.
# The last div contains the datatable.
...
app.layout = html.Div(children=[
    ...
    html.Div(children=[
        dash_table.DataTable(
            id='datatable',
            columns=[
                {"name": i, "id": i, "deletable": True, "selectable": True} for i in d.columns
            ],
...
## CALLBACKS
# The app has *three callbacks*. The first one 
# updates the graph when dropdown options are changed.
# The second one updatesthe hover text in the sidebar 
# whenever the user hovers on a different point.
# The third one selects a row in the datatable when 
# a point is clicked in the scatterplot.
...
def update_graph(x_col, y_col, color_col, highlight_StimID, color_scale,
    ...
    # if plot_click_data:
    #     point_index = plot_click_data['points'][0]['pointIndex']
    #     plotdf.iloc[point_index]['clicked'] = 'True'
    # fig.add_traces(
    #     px.scatter(plotdf.query(f'clicked == "True"'), 
    #             x=x, y=y, color=color)
    #             .update_traces(marker_size=25).data
    # )
