---
title: "Quantifying relevance: Data exploration write-up"
...
We can do this both for the normal and beta regression model
...
We test this hypothesis by a linear beta regression model (with maximal random effects) that regresses relevance ratings against the absolute difference between prior and posterior ratings.
We judge there to be evidence in favor of this hypothesis if the relevant slope coefficient is estimated to be credibly bigger than zero and a loo-based model comparison with an intercept only model substantially favors the model that inlcudes the relevant slope.
...
We also hypothesize that change in confidence ratings additionally contributes to predicting relevance ratings.
...
if (rerun_models) {
  ...
  write_rds(fit_confidence_diff, "cachedModels/fit_confidence_diff_no_interaction.rds")
  
} else {
  fit_confidence_diff <- read_rds("cachedModels/fit_confidence_diff_no_interaction.rds")
}

brms::hypothesis(fit_confidence_diff_no_interaction, "confidence_diff > 0")
loo_compare(list("w/__confidence" = loo(fit_confidence_diff_no_interaction),
                 "w/o_confidence" = loo(fit_belief_diff)))
```
...
```{r}
loo_compare(list("w/__interaction" = loo(fit_confidence_diff),
                 "w/o_interaction" = loo(fit_confidence_diff1)))
```

The expected log prob diff is less than the standard error diff, so I don't think there is evidence in favor of the interaction.
...
if (rerun_models) {
  ...
  write_rds(fit_second_order_diff, "cachedModels/fit_second_order_diff")
  
} else {
  fit_second_order_diff <- read_rds("cachedModels/fit_second_order_diff")
}
...
```{r}
loo_compare(list("second_order_diff" = loo(fit_second_order_diff),
                 "confidence_diff" = loo(fit_confidence_diff1)))
```
Yes. It seems to.
...
This also does not allow for beta regression.
So this is at best a quick first approximation of Bayes factor model comprison.
...
Here, we run single-factor Bayes factor model comparison. 
We use the default priors from the `ordbetareg` package.
