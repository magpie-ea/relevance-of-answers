...
cellComparisons <- tribble(
  ...
  "context: neutral > pos", ContextType_neutral_positive$post_prob, ContextType_neutral_positive$l_ci, ContextType_neutral_positive$u_ci,
  "context: neutral > neg", ContextType_neutral_negative$post_prob, ContextType_neutral_negative$l_ci, ContextType_neutral_negative$u_ci
)
...
loo_comp_hyp1 <- loo_compare(list("w/__beliefDiff" = loo(fit_belief_diff),
...
We also hypothesize that change in confidence (`second_order_belief_change`) ratings additionally contributes to predicting relevance ratings.
...
Concretely, we compare a model with single factor $X$ as a predictor to a model with predictors $X$, `pure_second_order_belief_change` and teir interaction.
...
loo_compare(fit_expHyp2_ER, fit_expHyp2_ER_pure)
loo_compare(fit_expHyp2_KL, fit_expHyp2_KL_pure)
loo_compare(fit_expHyp2_BF, fit_expHyp2_BF_pure)
...
model_names <- c("ER_ER", "ER_KL", "ER_BF",
  "KL_ER", "KL_KL", "KL_BF",
  "BF_ER", "BF_KL", "BF_BF")
