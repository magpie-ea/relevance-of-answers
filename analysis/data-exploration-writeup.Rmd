...
We find that the model without `group` has a Bayes factor of more than 900 in its favor.

```{r, results = 'hide'}
BFComp_group_noGroup / BFComp_group_full
```

Finally, let's also compare with LOO cross-validation, running the lesioned model first:
...
It appears that, when comparing trained models with REs, the leasioned model is slightly better numerically, but not with a margin large enough to strongly prefer it over the full model.
Nevertheless, we take all of this as an indication that we may proceed (temporarily, in visualizations) with partial neglect of the `trigger word` distinction. 
